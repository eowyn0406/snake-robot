//----- date: 2018-5-9
//----- version: v5.0
//----- notes: 修改控制指令，wsad控制头尾模块速度+偏航角，其余模块follow头/尾模块
...
void headTailCreator()
{
	...
	SnakeHead.geom = dCreateBox(space,ModuleLength/2,ModuleWidth/2,ModuleHeight/2);
	SnakeTail.geom = dCreateBox(space,ModuleLength/2,ModuleWidth/2,ModuleHeight/2);
	...
	dMassSetBox(&m,DensityOfModule,ModuleLength,ModuleWidth,ModuleHeight);

	dBodySetMass (SnakeHead.body,&m);
	dReal CenHead[] = {BodyX/2+ModuleLength/2+deltaX,deltaY,WheelRadius};
	...
	for(int i=0; i<NumOfHeadWheels; i++)
	{
		...
		dBodySetPosition(HeadWheelL[i].body,CenHead[0],CenHead[1]+ModuleWidth/2+WheelLength/2+0.1, CenHead[2]);
		dRFrom2Axes(Sp,0,0,1,1,0,0);	//分别以世界坐标系的Z,X轴作为轮子本体的X,Y轴
		dBodySetRotation(HeadWheelL[i].body,Sp);
		//dBodySetFiniteRotationMode(HeadWheelL[i].body,1);

		dBodySetMass (HeadWheelR[i].body,&m);
		dBodySetPosition(HeadWheelR[i].body,CenHead[0],CenHead[1]-ModuleWidth/2-WheelLength/2-0.1, CenHead[2]);
		dRFrom2Axes(Sp,0,0,1,1,0,0);	//分别以世界坐标系的Z,X轴作为轮子本体的X,Y轴
		dBodySetRotation(HeadWheelR[i].body,Sp);
		//dBodySetFiniteRotationMode(HeadWheelR[i].body,1);

		dBodySetMass (TailWheelL[i].body,&m);
		dBodySetPosition(TailWheelL[i].body,CenTail[0],CenTail[1]+ModuleWidth/2+WheelLength/2+0.1, CenTail[2]);
		dRFrom2Axes(Sp,0,0,1,1,0,0);	//分别以世界坐标系的Z,X轴作为轮子本体的X,Y轴
		dBodySetRotation(TailWheelL[i].body,Sp);
		//dBodySetFiniteRotationMode(TailWheelL[i].body,1);

		dBodySetMass (TailWheelR[i].body,&m);
		dBodySetPosition(TailWheelR[i].body,CenTail[0],CenTail[1]-ModuleWidth/2-WheelLength/2-0.1, CenTail[2]);
		dRFrom2Axes(Sp,0,0,1,1,0,0);	//分别以世界坐标系的Z,X轴作为轮子本体的X,Y轴
		dBodySetRotation(TailWheelR[i].body,Sp);
		//dBodySetFiniteRotationMode(TailWheelR[i].body,1);
	...
	for(int i=0; i<NumOfHeadWheels; i++)
	{
		...
		dJointSetHingeAnchor(HeadWheelLeftJoint[i],CenHead[0],deltaY+ModuleWidth/2+WheelLength/2+0.1, CenHead[2]);
		dJointSetHingeAxis(HeadWheelLeftJoint[i],0,1,0);
		dJointSetHingeAnchor(HeadWheelRightJoint[i],CenHead[0],deltaY-ModuleWidth/2-WheelLength/2-0.1, CenHead[2]);
		dJointSetHingeAxis(HeadWheelRightJoint[i],0,1,0);

		dJointSetHingeAnchor(TailWheelLeftJoint[i],CenTail[0],deltaY+ModuleWidth/2+WheelLength/2+0.1, WheelRadius);
		dJointSetHingeAxis(TailWheelLeftJoint[i],0,1,0);
		dJointSetHingeAnchor(TailWheelRightJoint[i],CenTail[0],deltaY-ModuleWidth/2-WheelLength/2-0.1, WheelRadius);
		dJointSetHingeAxis(TailWheelRightJoint[i],0,1,0);

		dJointSetHingeParam(HeadWheelLeftJoint[i],dParamBounce,0.01);
		dJointSetHingeParam(HeadWheelRightJoint[i],dParamBounce,0.01);
		dJointSetHingeParam(TailWheelLeftJoint[i],dParamBounce,0.01);
		dJointSetHingeParam(TailWheelRightJoint[i],dParamBounce,0.01);
	}
...
void bodyCreator()
{
	for(int i=0;i<NumOfModule;i++)
	{
		...
		SnakeBody[i].geom = dCreateBox(space,BodyX/2,BodyY/2,BodyZ/2);//生成几何形状
...
static void nearCallback(void *data, dGeomID o1, dGeomID o2)
{
	...
	if (isGround)  
	{
		...
		for (int i = 0; i < n; i++) 
		{
			...
			contact[i].surface.bounce     = 0.01; // (0.0~1.0) restitution parameter
...
static void start()
{
  static float xyz[3] = {0,10,20};
  static float hpr[3] = {-90, -50, 0};
...
static void simLoop(int pause)
{
	...
	dWorldStepFast1(world,stepsize,80);
	...
	dReal sidess[3] = {ModuleLength,ModuleWidth,ModuleHeight};
	dsSetColorAlpha(0,0.5,0.5,0.8);	//设置头模块的颜色
	...
	dsSetColorAlpha(1,1,1,0.8);	//设置尾模块的颜色
...
void controller(int steps, int ch)
{
	float desire_angle_head = 0, desire_angle_tail = 0;	//头、尾部模块偏航角，对应headjoint、tailjoint
	float diff_angle = 0;		//当前角度-目标角度
	...
	{	
		...
		case 97:
			...
			TLvel = -5;
			TRvel = -5;
			//desire_angle_head = (HLvel-HRvel)/(ModuleWidth/2+WheelLength/2+0.001)*Ta;
			//desire_angle_tail = 0;
			break;
		case 100:
			HLvel = -10;
		...
		int i = 0;
		diff_AR[0] = HLvel - dJointGetHingeAngleRate(HeadWheelLeftJoint[i]);
		diff_AR[1] = HRvel - dJointGetHingeAngleRate(HeadWheelRightJoint[i]);
		diff_AR[2] = TLvel - dJointGetHingeAngleRate(TailWheelLeftJoint[i]);
		diff_AR[3] = TRvel - dJointGetHingeAngleRate(TailWheelRightJoint[i]);
	}
	else
	{
		diff_angle = 0;
	...
	int i=0;
	dJointSetHingeParam(HeadWheelLeftJoint[i],dParamVel,0.8*diff_AR[0]);
	dJointSetHingeParam(HeadWheelLeftJoint[i],dParamFMax,maxtorque);
	dJointSetHingeParam(HeadWheelRightJoint[i],dParamVel,0.8*diff_AR[1]);
	dJointSetHingeParam(HeadWheelRightJoint[i],dParamFMax,maxtorque);
	dJointSetHingeParam(TailWheelLeftJoint[i],dParamVel,0.8*diff_AR[2]);
	dJointSetHingeParam(TailWheelLeftJoint[i],dParamFMax,maxtorque);
	dJointSetHingeParam(TailWheelRightJoint[i],dParamVel,0.8*diff_AR[3]);
	dJointSetHingeParam(TailWheelRightJoint[i],dParamFMax,maxtorque);

	diff_angle = desire_angle_head - dJointGetHingeAngle(headJoint);
	dJointSetHingeParam(headJoint,dParamVel,10*diff_angle);
	dJointSetHingeParam(headJoint,dParamFMax,maxtorque);
	diff_angle = desire_angle_tail - dJointGetHingeAngle(tailJoint);
	dJointSetHingeParam(tailJoint,dParamVel,10*diff_angle);
	dJointSetHingeParam(tailJoint,dParamFMax,maxtorque);


	float desire_angle_pitch = 0, desire_angle_yaw=0;
	switch (ch)
		{//119w,115s,97a,100d
		case 119:
			desire_angle_pitch = 0;
			desire_angle_yaw = 0;
			break;
		case 115:
			desire_angle_pitch = 0;
	...
	{
		if(i%2 == 1)
			diff_angle = desire_angle_yaw - dJointGetHingeAngle(BodyJoint[i]);
		else 
			diff_angle = desire_angle_pitch - dJointGetHingeAngle(BodyJoint[i]);
		dJointSetHingeParam(BodyJoint[i],dParamVel,10*diff_angle);
